...
I=dir("Twelve-Training\Twelve_train*.wav");
...
for i=1:length(I)
    ...
    Codewords{i}.ID=erase(I(i).name,["Twelve_train",".wav"]);
end

% Get the test data filepaths
I2=dir("Twelve-Testing\Twelve_test*.wav");
...
Actual=[string(erase(I2(1).name,["Twelve_test",".wav"]))];
for i=2:length(I2)
    Actual=[Actual, string(erase(I2(i).name,["Twelve_test",".wav"]))];
end
