...
% Elements per frame. 512 is sufficient for the 17/18 speakers in the
% training/testing datasets. Note that speaker 6 is consistently
% misidentified.
N=512;
% Distance between frame starts. 200 is sufficient for the 17/18 speakers in
% the training/testing datasets.
M=200;
% MFCC bins produced. 20 is sufficient for the 17/18 speakers in the
% training/testing datasets.
K=35;
% How many codewords to generate in the LBG algorithm, 2^NumIterations. 3
% is sufficient with the other settings for the 17/18 speakers in the
% training/testing datasets.
NumIterations=3;
...
for i=1:length(I)
    ...
    Codewords{i}.ID=erase(I(i).name,["Zero_train",".wav"]);
end
...
Actual=[string(erase(I2(1).name,["Zero_test",".wav"]))];
for i=2:length(I2)
    Actual=[Actual, string(erase(I2(i).name,["Zero_test",".wav"]))];
end
